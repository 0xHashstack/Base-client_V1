...
const generateLineChartData = (
	...
	let currentValue =
		startValue || minValue + Math.random() * (maxValue - minValue);
	...
	dates.forEach((date, index) => {
		...
		if (trend === 'up') {
			...
			currentValue = Math.max(
				minValue,
				Math.min(maxValue, currentValue + direction * change)
			);
		} else {
		...
		if (index > 0 && index % 7 === 0 && Math.random() > 0.7) {
			...
			currentValue = Math.max(
				minValue,
				Math.min(maxValue, currentValue + jumpDirection * jumpSize)
			);
		}
...
const generateStackedChartData = (
	...
	const currentValues = tokens.map(
		(_, i) => minValues[i] + Math.random() * (maxValues[i] - minValues[i])
	);
	...
	dates.forEach((date, dateIndex) => {
		...
		tokens.forEach((token, tokenIndex) => {
			...
			if (dateIndex > 0) {
				...
				const change =
					(Math.random() * 0.05 + 0.01) * currentValues[tokenIndex];
				currentValues[tokenIndex] = Math.max(
					minValues[tokenIndex],
					Math.min(
						maxValues[tokenIndex],
						currentValues[tokenIndex] + direction * change
					)
				);
				...
				if (dateIndex % 7 === 0 && Math.random() > 0.7) {
					...
					const jumpSize =
						...
						currentValues[tokenIndex];
					currentValues[tokenIndex] = Math.max(
						minValues[tokenIndex],
						Math.min(
							maxValues[tokenIndex],
							currentValues[tokenIndex] + jumpDirection * jumpSize
						)
					);
				}
			}
...
export const getProtocolInsightsData = () => {
	...
	const tvlData = generateDataForPeriods(generateLineChartData, [
		10000,
		800000,
		'up',
		768.86,
	...
	const borrowData = generateDataForPeriods(generateStackedChartData, [
		borrowTokens,
		[1000, 800, 500, 1200],
		[5000, 4000, 2500, 6000],
	]);
	...
	// Utilisation data
	const utilisationData = generateDataForPeriods(generateLineChartData, [
		50,
		100,
		'stable',
		92.8,
	...
	// Supply APR data
	const supplyAprData = generateDataForPeriods(generateLineChartData, [
		5,
		100,
		'volatile',
		98.345,
	...
	// Borrow APR data
	const borrowAprData = generateDataForPeriods(generateLineChartData, [
		10,
		60,
		'stable',
		43.56,
